...
describe('Main', function() {
			...
			chatClient2.close(() => {
				...
				chatClient3.close(async () => {
					...
					chatClient4.close(async () => {
						logger.log("...chatClient4 successfully disconnected.");
						if (LOCAL_STACK) {
							http_server.close();
							logger.log("HTTP Server closed.");
							webhook_app.close();
							logger.log("Webhooks endpoint closed.");
							logger.log("Waiting some seconds before stopping observer (it allows completing pending publish->ack).");
							await new Promise(resolve => setTimeout(resolve, 5000));
							observer.stopServer();
							logger.log("Waiting 1 second after observer stop.");
	...
	describe('TiledeskClient - Groups - test 3', function() {
		it('test 3 - Creates a group', function(done) {
			const group_id = "group-test3_" + uuid();
	...
	describe('TiledeskClient - Groups - test 4', function() {
		it('test 4 - Create group info messages. User1 creates a group with 2 members (user1, user2). User2 receives all the group info messages(user1.GROUP_CREATED, user1.MEMBER_JOINED_GROUP, user2.MEMBER_JOINED_GROUP)', function(done) {
			const group_id = "group-test4_" + uuid();
			const group_name = "Test Group " + group_id;
			const group_members = {}
			group_members[user2.userid] = 1;
			let messages = {}
	...
	describe('TiledeskClient - Groups - test 5', function() {
		it('test 5 - Send message to group. User1 creates a group. Group creator (User1) sends a message to the group and receives the sent message back.', function(done) {
			const group_id = "group-test5_" + uuid();
			const group_name = "Test Group " + group_id;
			const group_members = {}
				...
				let handler_message_added_client1 = _chatClient1.onMessageAdded((message, topic) => {
					...
					if (
						...
						message.text === SENT_MESSAGE
							...
							_chatClient1.close(() => {
								logger.log("..._chatClient1 successfully disconnected.");
								done();
							});
					}
				});
	...
	describe('TiledeskClient - Groups - test 6', function() {
		it('test 6 - Leave group. Creates group with 3 members, user1 creates group, user1 removes user3 from the group, user1 & user2 receive e group/clientupdate notification', function(done) {
			const group_id = "group-test6_" + uuid();
	...
	describe('TiledeskClient - Groups - test 7 - Join group with group-updated callback', function() {
		it('test 7 - Join group (group/clientupdate notification subscription). Creates group with 2 members. User1 creates group, user1 (owner) adds user3 to the group, user1 & user2 receive e group/clientupdate notification', function(done) {
			const group_id = "group-test7_" + uuid();
			...
			chatClient1.onGroupUpdated((group, topic) => {
				...
				if (group.uid === group_id) {
					logger.log("test 7 - chatClient1 - group updated (got group.uid):", group_id);
			...
			chatClient2.onGroupUpdated((group, topic) => {
				...
				if (group.uid === group_id) {
					logger.log("test 7 - chatClient2 - group updated (got group.uid):", group_id);
					if (group.members[user3.userid]) {
	...
	describe('TiledeskClient - test 8 - Join group with JOINED_MEMBER message notification', function() {
		it('test 8 - Join group (group info messages). User1 (owner) creates a group with 2 members (User1, User2). User1 adds User3 to the group, user1 & user2 receive e MEMBER_JOINED_GROUP message', function(done) {
			const group_id = "group-test8_" + uuid();
			const group_name = "group-update test 8";
			let group_members = {};
			group_members[user1.userid] = 1;
			group_members[user2.userid] = 1;
	...
	describe('TiledeskClient - Groups - test 9 - Join group - full notifications (group-update, JOINED_MEMBER messages)', function() {
		it('test 9 - Join group, complete notifications (testing receiving of both info messages & group/update notifications). User1 creates a group with 2 members. User1 adds user3 to the group, user1 & user2 receive e MEMBER_JOINED_GROUP message & group/updated notification', function(done) {
			const group_id = "group-test9_" + uuid();
			...
			_chatClient1.connect(user1.userid, user1.token, () => {
				...
				_chatClient2.connect(user1.userid, user1.token, () => {
					...
					let handler_message_added_client2 = _chatClient2.onMessageAdded((message, topic) => {
						if (message.recipient === group_id) {
							logger.log("test 9 - message added:", JSON.stringify(message));
							if (
								message &&
					...
					let handler_group_updated_client1 = _chatClient1.onGroupUpdated((group, topic) => {
						if (group.uid === group_id) {
							logger.log("test 9 - group updated - _chatClient1:", JSON.stringify(group));
					...
					let handler_group_updated_client2 = _chatClient2.onGroupUpdated((group, topic) => {
						if (group.uid === group_id) {
							logger.log("test 9 - group updated - _chatClient2:", JSON.stringify(group));
							if (group.notification &&
								group.notification.messagelabel &&
								group.notification.messagelabel.key === "MEMBER_JOINED_GROUP" &&
								group.notification.messagelabel.parameters &&
					...
					function CheckAndDone() {
						if (update_notifications['message_' + user1.userid] === 1 &&
							...
							update_notifications['group_update_notification_' + user2.userid] === 1) {
								...
								console.log("removing handler handler_message_added_client1:", handler_message_added_client1);
								console.log("removing handler handler_message_added_client2:", handler_message_added_client2);
								console.log("removing handler handler_group_updated_client1:", handler_group_updated_client1);
								console.log("removing handler handler_group_updated_client2:", handler_group_updated_client2);
								_chatClient1.removeOnMessageAddedHandler(handler_message_added_client1);
								_chatClient1.removeOnMessageAddedHandler(handler_message_added_client2);
								_chatClient1.removeOnGroupUpdatedHandler(handler_group_updated_client1);
								_chatClient1.removeOnGroupUpdatedHandler(handler_group_updated_client2);
								_chatClient1.close(() => {
									logger.log("..._chatClient1 successfully disconnected.");
									_chatClient2.close(() => {
										logger.log("..._chatClient2 successfully disconnected.");
										done();
									});
								});
						}
...
User3 receives the (only) message previously sent by User1 to the group', function(done) {
			const group_id = "group-test10_" + uuid();
...
// 			const group_id = "group-test11_" + uuid();
// 			const group_name = "group-update test";
// 			const MESSAGE1_USER1 = "test 11, user1, first";
// 			let group_members = {};
// 			group_members[user1.userid] = 1;
// 			group_members[user2.userid] = 1;
...
// 							}
// 							logger.log("Message sent:", msg);
// 							await new Promise(resolve => setTimeout(resolve, 2000)); // it gives time to join message to reach the "persistent" status
// 							chatClient1.groupJoin(group_id, user3.userid, (err, json) => {
// 								if (err) {
...
User2, removed, will receive group-update notification (original member)', function(done) {
			const group_id = "group-test12_" + uuid();
			...
			chatClient1.onGroupUpdated((group, topic) => {
				...
				if (group.uid === group_id) {
					update_notifications[user1.userid] = 1;
					if (update_notifications[user1.userid] === 1 && update_notifications[user2.userid] === 1) {
						logger.log("test 12 - ALL NOTIFICATIONS RECEIVED");
						done();
					}
				}
			});
				...
				async (err, result) => {
					...
					chatClient1.groupSetMembers(
						...
						async (err, result) => {
							...
							chatClient1.groupData(group_id, (err, json) => {
								logger.log("test 12 - group updated:", group_id, "data:", json);
								assert(err == null);
								assert(json != null);
								assert(json.success == true);
								assert(json.result != null);
